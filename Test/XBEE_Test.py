...
Enter AT <enter>
The module should reply ‘OK’
If you do not get a response from module, probe the Reset pin (pin 5 on J6 left side of XBee). 
It should be high. If it is low then enter the command to enable Xbee reset. Then probe both UART lines. 
Both should be high. If they were being pulled low then something may be not functional. 
...
If this is not working then there may be an issue with the Octavo OR the UART multiplexer on UART4 
may be set to a different destination, e.g. Expansion Port.
